...
class GmapPlacePicker extends StatefulWidget {
  // static const MethodChannel _channel = const MethodChannel('gmap_place_picker');
  //
  ...
  @override
  GmapPlacePickerState createState() => GmapPlacePickerState();
}

class GmapPlacePickerState extends State<GmapPlacePicker> {
  ...
  void autoCompleteSearch(String place) {
    ...
    String regionParam = countries?.isNotEmpty == true ? "&components=country:${countries.sublist(0, min(countries.length, 5)).join('|country:')}" : "";

    var endpoint = "https://maps.googleapis.com/maps/api/place/autocomplete/json?" + "key=${widget.apiKey}&" + "input={$place}$regionParam&sessiontoken=$sessionToken&" + "language=${widget.language}";

    if (placeInfo != null) {
      endpoint += "&location=${placeInfo.latLng.latitude}," + "${placeInfo.latLng.longitude}";
    }

    // LocationUtils.getAppHeaders().then((headers) =>
    //     http.get(Uri.parse(endpoint), headers: headers)).then((response) {
  ...
  void decodeAndSelectPlace(String placeId) {
    ...
    String endpoint = "https://maps.googleapis.com/maps/api/place/details/json?key=${widget.apiKey}" + "&placeid=$placeId" + '&language=${widget.language}';
...
//  }
  ...
  void getNearbyPlaces(LatLng latLng) {
    // LocationUtils.getAppHeaders().then((headers) =>
    //     http.get(Uri.parse("https://maps.googleapis.com/maps/api/place/nearbysearch/json?" + "key=${widget.apiKey}&" + "location=${latLng.latitude},${latLng.longitude}&radius=150" + "&language=${widget.language}"), headers: headers)).then((response) {
    http
        .get(
      Uri.parse("https://maps.googleapis.com/maps/api/place/nearbysearch/json?" + "key=${widget.apiKey}&" + "location=${latLng.latitude},${latLng.longitude}&radius=150" + "&language=${widget.language}"),
    )
        .then((response) {
      if (response.statusCode == 200) {
        nearbyPlaces.clear();
  ...
  Future reverseGeocodeLatLng(LatLng latLng) async {
    // var response = await http.get(Uri.parse("https://maps.googleapis.com/maps/api/geocode/json?latlng=${latLng.latitude},${latLng.longitude}" + "&key=${widget.apiKey}" + "&language=${widget.language}"), headers: await (LocationUtils.getAppHeaders()));
    var response = await http.get(Uri.parse("https://maps.googleapis.com/maps/api/geocode/json?latlng=${latLng.latitude},${latLng.longitude}" + "&key=${widget.apiKey}" + "&language=${widget.language}"));
...
///
///
Future<PlaceInfo> openPlacePicker(
