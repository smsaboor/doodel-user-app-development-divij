...
class OngoingOrdersTab extends StatelessWidget {
  const OngoingOrdersTab(this.userID, {Key key}) : super(key: key);
  final String userID;

  @override
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance.collection(orders_Collection).where('userUid', isEqualTo: userID).snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.data.docs.isEmpty) {
              return Center(child: Text('No Ongoing Order found'));
            }
